...
import { fetchFlashNewsAdminList } from '../../../lib/admin-gql.ts';
...
export async function render(ctx: NexusContext) {
  ...
  const rows = (await fetchFlashNewsAdminList(ctx)) ?? [];
  const newHref = pathWithLang(ctx, '/admin/flashes/new');
  ...
  const empty = rows.length === 0 ? `<p class="pf-admin-muted">${esc(t.flashEmpty)}</p>` : '';

  const html = `
  <section class="pf-admin-page pf-admin-flash-list">
    <header class="pf-admin-head pf-admin-head--row">
      <h1 class="pf-admin-h1">${esc(t.flashListTitle)}</h1>
      <a class="pf-admin-btn" href="${esc(newHref)}">${esc(t.flashNew)}</a>
    </header>
    ${empty}
    ${
      rows.length > 0
  ...
  <style>
    .pf-admin-flash-list .pf-admin-head--row {
      display: flex;
      flex-wrap: wrap;
      align-items: center;
