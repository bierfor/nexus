...
import { fetchArticlesAdminList } from '../../../lib/admin-gql.ts';
...
export async function render(ctx: NexusContext) {
  ...
  const rows = (await fetchArticlesAdminList(ctx)) ?? [];
  const newHref = pathWithLang(ctx, '/admin/articles/new');
  ...
  const empty = rows.length === 0 ? `<p class="pf-admin-muted">${esc(t.articleEmpty)}</p>` : '';

  const html = `
  <section class="pf-admin-page pf-admin-articles-list">
    <header class="pf-admin-head pf-admin-head--row">
      <h1 class="pf-admin-h1">${esc(t.articleListTitle)}</h1>
      <a class="pf-admin-btn" href="${esc(newHref)}">${esc(t.articleNew)}</a>
    </header>
    ${empty}
    ${
      rows.length > 0
  ...
  <style>
    .pf-admin-articles-list .pf-admin-head--row {
      display: flex;
      flex-wrap: wrap;
      align-items: center;
